...
gen.addParams({
    "avoid_addr_range_min_value" : args.node3minAddr * 1024 * 1024,
    "avoid_addr_range_max_value" : args.node3maxAddr * 1024 * 1024,
...
connect("mirandaCPU_%d_l1_cache_%d_link"%(pe_id, rtr_id),
        pe, "cache_link",
        arbiter, "port3",
...
# Memory controller with the address range node3maxAddr to total_memory_size_in_MB
print("\tBuilding mem_ctrl_%d" % mem_id)
mem = sst.Component("mem_ctrl_%d" % mem_id, "memHierarchy.MemController")
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node3maxAddr * 1024 * 1024,
    "addr_range_end": total_memory_size_in_MB * 1024 * 1024})

mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((total_memory_size_in_MB-args.node3maxAddr) * 1024 * 1024) + "B"})
mem_backend.addParams(mem_backend_params)

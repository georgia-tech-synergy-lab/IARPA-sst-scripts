...
parser.add_argument('--node5minAddr', type=int, default=500)
parser.add_argument('--node6minAddr', type=int, default=600)
parser.add_argument('--node7minAddr', type=int, default=700)
parser.add_argument('--node8minAddr', type=int, default=800)
parser.add_argument('--node9minAddr', type=int, default=900)
...
parser.add_argument('--node5maxAddr', type=int, default=600)
parser.add_argument('--node6maxAddr', type=int, default=700)
parser.add_argument('--node7maxAddr', type=int, default=800)
parser.add_argument('--node8maxAddr', type=int, default=900)
parser.add_argument('--node9maxAddr', type=int, default=1000)
parser.add_argument('--packet_count', type=int, default=1)
parser.add_argument('--max_reqs_per_cycle', type=int, default=2)

args = parser.parse_args()

exp_name = "Arbiter-Scenario5"

num_mem_ctrls = 5
num_pes = 3 # Number of Processing Engines (CPUs)
core_clock = "2800MHz"
cache_line_size = 64
...
arbiter2_params = {
    "arbiter_frequency": core_clock,
    "port1minAddr": args.node4minAddr * 1024 * 1024,
    "port2minAddr": args.node5minAddr * 1024 * 1024,
    "port3minAddr": args.node6minAddr * 1024 * 1024,
    "port1maxAddr": args.node4maxAddr * 1024 * 1024,
    "port2maxAddr": args.node5maxAddr * 1024 * 1024,
    "port3maxAddr": args.node6maxAddr * 1024 * 1024,
}

arbiter3_params = {
    "arbiter_frequency": core_clock,
    "port1minAddr": args.node7minAddr * 1024 * 1024,
    "port2minAddr": args.node8minAddr * 1024 * 1024,
    "port3minAddr": args.node9minAddr * 1024 * 1024,
    "port1maxAddr": args.node7maxAddr * 1024 * 1024,
    "port2maxAddr": args.node8maxAddr * 1024 * 1024,
    "port3maxAddr": args.node9maxAddr * 1024 * 1024,
}
...
arbiter.addParams(arbiter1_params)
...
#gen = pe.setSubComponent("generator", "miranda.GUPSGenerator")
gen.addParams({
    "seed_a"                     : 10,
    "seed_b"                     : 11,
    "avoid_addr_range_min_value" : args.node1minAddr * 1024 * 1024,
    "avoid_addr_range_max_value" : args.node1maxAddr * 1024 * 1024,
    "avoid_addr_range_min_value1" : args.node5minAddr * 1024 * 1024,
    "avoid_addr_range_max_value1" : args.node5maxAddr * 1024 * 1024,
    "avoid_addr_range_min_value2" : args.node9minAddr * 1024 * 1024,
    "avoid_addr_range_max_value2" : args.node9maxAddr * 1024 * 1024,
...
print("\t\tCreating mirandaCPU_%d_arbiter_%d_link"%(pe_id, rtr_id))
connect("mirandaCPU_%d_arbiter_%d_link"%(pe_id, rtr_id),
        pe, "cache_link",
        arbiter, "port1",
...
mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((args.node3maxAddr-args.node3minAddr) * 1024 * 1024) + "B"}) # For DRAMSim3, backend.mem_size must be a multiple of 1MB.
mem_backend.addParams(mem_backend_params)
...
print("\tBuilding arbiter_%d" % (rtr_id))
arbiter = sst.Component("arbiter_%d" % (rtr_id), "memHierarchy.RoundRobinArbiter")
arbiter.addParams(arbiter2_params)
...
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node4minAddr * 1024 * 1024,
    "addr_range_end": args.node4maxAddr * 1024 * 1024})
mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((args.node4maxAddr-args.node4minAddr) * 1024 * 1024) + "B"})
mem_backend.addParams(mem_backend_params)
...
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node6minAddr * 1024 * 1024,
    "addr_range_end": args.node6maxAddr * 1024 * 1024
    })
mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((args.node6maxAddr-args.node6minAddr) * 1024 * 1024) + "B"}) # For DRAMSim3, backend.mem_size must be a multiple of 1MB.
mem_backend.addParams(mem_backend_params)
...
print("\tBuilding arbiter_%d" % (rtr_id))
arbiter = sst.Component("arbiter_%d" % (rtr_id), "memHierarchy.RoundRobinArbiter")
arbiter.addParams(arbiter3_params)
...
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node7minAddr * 1024 * 1024,
    "addr_range_end": args.node7maxAddr * 1024 * 1024
    })
mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((args.node7maxAddr-args.node7minAddr) * 1024 * 1024) + "B"}) # For DRAMSim3, backend.mem_size must be a multiple of 1MB.
mem_backend.addParams(mem_backend_params)
...
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node8minAddr * 1024 * 1024,
    "addr_range_end": args.node8maxAddr * 1024 * 1024})
mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((args.node8maxAddr-args.node8minAddr) * 1024 * 1024) + "B"})
mem_backend.addParams(mem_backend_params)
...
# Memory controller with the address range node9maxAddr to total_memory_size_in_MB
print("\tBuilding mem_ctrl_%d" % mem_id)
mem = sst.Component("mem_ctrl_%d" % mem_id, "memHierarchy.MemController")
mem.addParams(mem_params)
mem.addParams({
    "addr_range_start": args.node9maxAddr * 1024 * 1024,
    "addr_range_end": total_memory_size_in_MB * 1024 * 1024})

mem_backend = mem.setSubComponent("backend", "memHierarchy.dramsim3")
mem_backend.addParams({"mem_size": str((total_memory_size_in_MB-args.node9maxAddr) * 1024 * 1024) + "B"})
mem_backend.addParams(mem_backend_params)
